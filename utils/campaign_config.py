...
class CampaignEvalO3():
    ...
    def __init__(self, config_file, cmp_row_no):
        ...
        Returns:
            None
        """
        self.spark = SparkSession.builder.appName(f"campaignEvalO3").getOrCreate()
        self.spark.sparkContext.setCheckpointDir("dbfs:/mnt/pvtdmbobazc01/edminput/filestore/user/thanakrit_boo/tmp/checkpoint")
        self.spark.conf.set("spark.databricks.io.cache.enabled", True)
        self.spark.conf.set("spark.databricks.queryWatchdog.maxQueryTasks", 0)
        self.spark.conf.set("spark.databricks.queryWatchdog.outputRatioThreshold", 20000)
