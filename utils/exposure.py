...
def get_exposure(cmp, exposure_type: str, visit_multiplier: float):
    ...
    cmp.params["visit_multiplier"] = visit_multiplier
    ...
    if exposure_type == "store_lv":
        ...
        cmp.params["exposure_type"] = "undefined"
        exposure = None
        return exposure
...
def get_awareness(cmp: CampaignEval):
    ...
    (cmp.txn
     ...
     .join(cmp.aisle_sku, 'upc_id', 'inner')
     .fillna(str(cmp.cp_start_date), subset='c_start')
     .fillna(str(cmp.cp_end_date), subset='c_end')
    ...
    count_test_store_all =  cmp.target_store.select('store_id').drop_duplicates().count()
