...
def get_cust_brand_switching_and_penetration(cmp):
    ...
    def _switching(
        ...
        cust_micro_df = cust_movement_sf.where(
            F.col("customer_micro_flag") == micro_flag
        )
        prod_trans_cust_micro = prod_trans.join(
            cust_micro_df.select("household_id").dropDuplicates(),
            on="household_id",
            how="inner",
        )
        cust_micro_kpi_prod_lv = (
            prod_trans_cust_micro.groupby(grp)
            .agg(
                F.sum("net_spend_amt").alias("oth_" + prod_lev + "_spend"),
        ...
        total_oth = (
            cust_micro_kpi_prod_lv.agg(
                F.sum("oth_" + prod_lev + "_spend").alias("_total_oth_spend")
            )
        ).collect()[0][0]
        ...
        cust_micro_kpi_prod_lv = cust_micro_kpi_prod_lv.withColumn(
            "total_oth_" + prod_lev + "_spend", F.lit(float(total_oth))
        )

        print("\t\t\t\t\t\t**Running micro df2")
        # Join micro df with prod trans
        if (prod_lev == "brand") & (switching_lv == "subclass"):
            cust_micro_df2 = cust_micro_df.groupby(
                "division_name",
                "department_name",
                "section_name",
                "class_name",
                ...
                F.sum("brand_spend_" + period).alias("total_ori_brand_spend"),
                F.countDistinct("household_id").alias("total_ori_brand_cust"),
            )
            micro_df_summ = cust_micro_df2.join(
                cust_micro_kpi_prod_lv, on="class_name", how="inner"
            )
        ...
        switching_result = (
            micro_df_summ.select(
                ...
                "customer_macro_flag",
                "customer_micro_flag",
                "total_ori_brand_cust",
                "total_ori_brand_spend",
                "oth_" + full_prod_lev,
                "oth_" + prod_lev + "_customers",
                "oth_" + prod_lev + "_spend",
                "total_oth_" + prod_lev + "_spend",
            )
            .withColumn(
                "pct_cust_oth_" + full_prod_lev,
                F.col("oth_" + prod_lev + "_customers") / F.col("total_ori_brand_cust"),
            )
            .withColumn(
                "pct_spend_oth_" + full_prod_lev,
                F.col("oth_" + prod_lev + "_spend")
                / F.col("total_oth_" + prod_lev + "_spend"),
            )
    ...
    def _get_swtchng_pntrtn(switching_lv: str):
        ...
        prior_pre_cc_txn_prd_scope_sel_brand = prior_pre_cc_txn_prd_scope.join(
            brand_df, "upc_id", "inner"
        )

        prior_pre_prd_scope_sel_brand_kpi = (
            prior_pre_cc_txn_prd_scope_sel_brand.groupBy(gr_col).agg(
                F.sum("net_spend_amt").alias("brand_spend_pre")
            )
        )
        ...
        dur_cc_txn_prd_scope_sel_brand = dur_cc_txn_prd_scope.join(
            brand_df, "upc_id", "inner"
        )

        dur_prd_scope_sel_brand_kpi = dur_cc_txn_prd_scope_sel_brand.groupBy(
            gr_col
        ).agg(F.sum("net_spend_amt").alias("brand_spend_dur"))

        pre_dur_band_spend = prior_pre_prd_scope_sel_brand_kpi.join(
            dur_prd_scope_sel_brand_kpi, gr_col, "outer"
        )

        cust_movement_pre_dur_spend = cust_movement_sf.join(
            pre_dur_band_spend, "household_id", "left"
        )
        new_to_brand_switching_from = _switching(
            switching_lv,
            "new_to_brand",
        ...
        dur_prd_scope_cust = dur_cc_txn_prd_scope.agg(
            F.countDistinct("household_id")
        ).collect()[0][0]
        brand_cust_pen = (
            dur_cc_txn_prd_scope.groupBy("brand_name")
    ...
    new_to_brand_switching, cust_mv_pre_dur_spend, brand_cust_pen = _get_swtchng_pntrtn(
        switching_lv=switching_lv
    )
    cust_brand_switching_and_pen = (
        new_to_brand_switching.alias("a")
        .join(
            brand_cust_pen.alias("b"),
            F.col("a.oth_brand_in_category") == F.col("b.brand_name"),
            "left",
        )
...
def get_cust_brand_switching_and_penetration_multi(cmp):
    ...
    new_to_brand_cust = cust_movement_sf.where(
        F.col("customer_micro_flag") == "new_to_brand"
    )
    n_new_to_brand_cust = (
        cust_movement_sf.where(F.col("customer_micro_flag") == "new_to_brand")
        .agg(F.count_distinct("household_id"))
        .collect()[0][0]
    )
    ...
    combine_hier = (
        prior_pre_new_to_brand_txn_in_cate.select(
            "brand_name",
            F.concat_ws(
                "_",
                "division_name",
    ...
    pre_new_to_brand_cate_cust = prior_pre_new_to_brand_txn_in_cate.agg(
        F.count_distinct("household_id")
    ).collect()[0][0]
    pre_brand_in_cate = (
        prior_pre_new_to_brand_txn_in_cate.groupBy("brand_name")
        .agg(F.count_distinct("household_id").alias("pre_brand_switch_cust"))
        .withColumn("new_to_brand_cust", F.lit(n_new_to_brand_cust))
        .withColumn(
            "prop_cust_switch",
            F.col("pre_brand_switch_cust") / F.col("new_to_brand_cust"),
        )
    )
    ...
    pre_cate_cust = prior_pre_txn_in_cate.agg(
        F.count_distinct("household_id")
    ).collect()[0][0]
    pre_brand_cust_pen = (
        prior_pre_txn_in_cate.groupBy("brand_name")
...
def get_cust_sku_switching(cmp):
    ...
    txn_per_dur_cat_sale = (
        ...
        .withColumn(
            "pre_cat_sales",
            F.when(
                F.col(period_wk_col_nm).isin(["ppp", "pre"]), F.col("net_spend_amt")
            ).otherwise(0),
        )
        .withColumn(
            "dur_cat_sales",
            F.when(
                F.col(period_wk_col_nm).isin(["dur"]), F.col("net_spend_amt")
            ).otherwise(0),
        )
        .withColumn(
            "cust_tt_pre_cat_sales",
            F.sum(F.col("pre_cat_sales")).over(Window.partitionBy("household_id")),
        )
        .withColumn(
            "cust_tt_dur_cat_sales",
            F.sum(F.col("dur_cat_sales")).over(Window.partitionBy("household_id")),
        )
    )

    txn_cat_both = txn_per_dur_cat_sale.where(
        (F.col("cust_tt_pre_cat_sales") > 0) & (F.col("cust_tt_dur_cat_sales") > 0)
    )

    txn_cat_both_sku_only_dur = (
        txn_cat_both.withColumn(
            "pre_sku_sales",
            F.when(
                (F.col(period_wk_col_nm).isin(["ppp", "pre"]))
                & (F.col("upc_id").isin(feat_list)),
                F.col("net_spend_amt"),
            ).otherwise(0),
        )
        .withColumn(
            "dur_sku_sales",
            F.when(
                (F.col(period_wk_col_nm).isin(["dur"]))
                & (F.col("upc_id").isin(feat_list)),
                F.col("net_spend_amt"),
            ).otherwise(0),
        )
        .withColumn(
            "cust_tt_pre_sku_sales",
            F.sum(F.col("pre_sku_sales")).over(Window.partitionBy("household_id")),
        )
        .withColumn(
            "cust_tt_dur_sku_sales",
            F.sum(F.col("dur_sku_sales")).over(Window.partitionBy("household_id")),
        )
        .where(
            (F.col("cust_tt_pre_sku_sales") <= 0) & (F.col("cust_tt_dur_sku_sales") > 0)
        )
    )

    n_cust_switch_sku = (
        txn_cat_both_sku_only_dur.where(
            F.col("pre_cat_sales") > 0
        )  # only other products
