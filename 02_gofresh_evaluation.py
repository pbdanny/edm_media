...
from instore_eval import get_cust_activated, get_cust_movement, get_cust_brand_switching_and_penetration, get_cust_sku_switching, get_profile_truprice, get_customer_uplift, get_cust_activated_prmzn
...
brand_activated, sku_activated = get_cust_activated(txn=txn_all,
...
## temp cell just to load cust table will be remove later as it generate from pre step already

# cust_mv = spark.table(f'tdm_seg.media_camp_eval_{cmp_id}_cust_mv')

#print(f'tdm_seg.media_camp_eval_{cmp_id}_cust_mv')
...
# cust_mv.groupBy( cust_mv.customer_macro_flag
#                 ,cust_mv.customer_micro_flag
#                )\
#        .agg    ( count('household_id').alias('cc_cnt')
#                 ,countDistinct('household_id').alias('cc_cnt_unique')
#                )\
#        .display()
...
cate_info_df, cate_brand_info = get_new_to_cate(txn_all,cust_mv, 'promo_wk' )
...
# MAGIC %md # Sales growth & Marketshare growth (Pre/During) -- Add from AE request to see some more metrics for standard
...
# sku_trend_trg   = get_sales_trend_trg(txn_all, trg_str_df, feat_df, 'SKU', 'promo_wk', 'period_promo_wk')

## Brand sales trend
# brand_trend_trg = get_sales_trend_trg(txn_all, trg_str_df, brand_df, 'Brand', 'promo_wk', 'period_promo_wk')

## Category sales trend
# cate_trend_trg  = get_sales_trend_trg(txn_all, trg_str_df, cate_df, 'Category', 'promo_wk', 'period_promo_wk')
