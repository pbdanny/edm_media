...
from instore_eval import get_cust_activated, get_cust_movement, get_cust_brand_switching_and_penetration, get_cust_sku_switching, get_profile_truprice, get_customer_uplift, get_cust_activated_prmzn, check_combine_region
...
pre_st_promo_mv_wk = promo_week_cal(pre_en_promo_wk, -12)   ## pre period = 13 weeks
...
try:
    ...
    print(f'Create intermediate transaction table for period Pre - Dur (Promo Week), all store format : tdm_seg.media_campaign_eval_txn_data_{cmp_id}')
...
# MAGIC %md
# MAGIC ## Standard Report : Awareness -- No Awareness for Promo eval
